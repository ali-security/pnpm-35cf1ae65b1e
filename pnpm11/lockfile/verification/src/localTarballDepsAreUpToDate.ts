...
async function checkLocalTarballDeps (
  ...
  const results = await Promise.all(DEPENDENCIES_FIELDS.map((depField) => {
    if (includedDependencies?.[depField] === false) {
      return { kind: 'up-to-date' } as const
    }
    return checkLocalTarballDepsField({
      fileIntegrityCache,
      lockfilePackages,
      lockfileDir,
    }, project.snapshot[depField])
  }))
  return results.find((result) => result.kind !== 'up-to-date') ?? { kind: 'up-to-date' }
}

async function checkLocalTarballDepsField (
  {
    fileIntegrityCache,
    lockfilePackages,
    lockfileDir,
  }: Omit<LocalTarballDepsUpToDateContext, 'includedDependencies'>,
  lockfileDeps: ProjectSnapshot[keyof Pick<ProjectSnapshot, 'dependencies' | 'devDependencies' | 'optionalDependencies'>]
): Promise<LocalTarballDepsCheckResult> {
